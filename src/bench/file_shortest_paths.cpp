...
{
    int ret = 0;
    struct settings s = { DEFAULT_NTHREADS, "", DEFAULT_SEED, 1000000000, DEFAULT_OUTPUT_FILE, PQ_MULTIQC1, DEFAULT_BETA};

    int opt;
    while ((opt = getopt(argc, argv, "i:n:w:o:p:s:b:")) != -1) {
        switch (opt) {
        case 'i':
            errno = 0;
            s.graph_file = optarg;
    ...
    if (s.type == PQ_CADM) {
        ...
        ret = bench(&pq, s);
    } else if (s.type == PQ_MULTIQC1) {
        kpqbench::multiq<size_t, size_t, 1> pq(s.num_threads, s.beta);
        ret = bench(&pq, s);
    } else if (s.type == PQ_MULTIQC2) {
        kpqbench::multiq<size_t, size_t, 2> pq(s.num_threads, s.beta);
        ret = bench(&pq, s);
    } else if (s.type == PQ_MULTIQC4) {
        kpqbench::multiq<size_t, size_t, 4> pq(s.num_threads, s.beta);
        ret = bench(&pq, s);
    } else if (s.type == PQ_MULTIQC8) {
        kpqbench::multiq<size_t, size_t, 8> pq(s.num_threads, s.beta);
        ret = bench(&pq, s);
    } else if (s.type == PQ_MULTIQC16) {
        kpqbench::multiq<size_t, size_t, 16> pq(s.num_threads, s.beta);
        ret = bench(&pq, s);
    } else if (s.type == PQ_MULTIQC32) {
        kpqbench::multiq<size_t, size_t, 32> pq(s.num_threads, s.beta);
        ret = bench(&pq, s);
    } else if (s.type == PQ_MULTIQC64) {
        kpqbench::multiq<size_t, size_t, 64> pq(s.num_threads, s.beta);
        ret = bench(&pq, s);
    }  else if (s.type == PQ_MULTIQC128) {
        kpqbench::multiq<size_t, size_t, 128> pq(s.num_threads, s.beta);
        ret = bench(&pq, s);
    }  else if (s.type == PQ_MULTIQC256) {
        kpqbench::multiq<size_t, size_t, 256> pq(s.num_threads, s.beta);
        ret = bench(&pq, s);
